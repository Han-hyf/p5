...
$.param                     object转param，{a:1,b:2} => a=1&b=2
$.tools.url                 获取location.search全部参数
$.tools.Cookie              cookie操作
$.tools.fullNumber          数字<10补0
$.tools.datePicker          日期选择
...
jQuery.extend({
    tools: {
        ...
        @type            默认全功能：日期，时分秒；1 没有时间选择 2 没有秒选择

        --------
        模板在footer.vm中，id为datePicker
        */
        'datePicker': function(el, tmpl, options) {
            ...
            // 初始input状态
//            el.attr('readonly',true).css({'cursor':'pointer'});
            el.css({'cursor':'pointer'});
            if($.trim(el.attr('placeHolder')) === ''){
                el.attr('placeHolder','选择日期');
            }
            // 清空按钮
            ...
            function render() {
                ...
                picker.html($.tmpl(tmpl, data)).css({
                    top: top,
                    left: el.offset().left,
                    display: 'block',
                    position: 'absolute',
