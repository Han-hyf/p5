...
/**
 * Created by zhuwei on 2015/5/13.
 */
...
public class IdCard {
    ...
    public static void main(String[] args) {
        String id = "130105195801040651";
        IdCard icu = new IdCard(id);
        System.out.println(icu.validate());
    }

    public boolean validate() {
        if (null == cacheValidateResult) {
            boolean result = (null != cardNumber);// 身份证号不能为空
            ...
            // 身份证号的第18位校验正确
            char code = calculateVerifyCode(cardNumber);
            result = result && (code == cardNumber.charAt(NEW_CARD_NUMBER_LENGTH - 1)
                    || code == cardNumber.charAt(NEW_CARD_NUMBER_LENGTH - 1) + 32); // 不区分大小写
            ...
            cacheValidateResult = result;// TODO 完整身份证号码的省市县区检验规则
        }
