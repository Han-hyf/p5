...
public class FullyQualifiedJavaType implements
    ...
    private static HashMap<String, FullyQualifiedJavaType> criteriaInstanceMap = null;
    ...
    public FullyQualifiedJavaType(String baseShortName, boolean is) {
        super();
        typeArguments = new ArrayList<FullyQualifiedJavaType>();
    ...
    public static final FullyQualifiedJavaType getNewListParamInstance() {
        // always return a new instance because the type may be parameterized
        return new FullyQualifiedJavaType("java.util.List<?>"); //$NON-NLS-1$
    }
    ...
    public static final FullyQualifiedJavaType getNewHashMapInstance() {
        // always return a new instance because the type may be parameterized
        return new FullyQualifiedJavaType("java.util.HashMap<String, ?>"); //$NON-NLS-1$
    }
    ...
    public static final FullyQualifiedJavaType getNewArrayListParamInstance() {
        // always return a new instance because the type may be parameterized
        return new FullyQualifiedJavaType("java.util.ArrayList<?>"); //$NON-NLS-1$
    }
    ...
    public static final FullyQualifiedJavaType getByName(String typeName) {
        if (null == criteriaInstanceMap) {
            criteriaInstanceMap = new HashMap<>();
        }
        if (criteriaInstanceMap.get(typeName) == null) {
            criteriaInstanceMap.put(typeName, new FullyQualifiedJavaType(typeName, true));
        }

        return criteriaInstanceMap.get(typeName);
    }
    ...
    private void simpleParse(String typeSpecification) {
        ...
        if (baseQualifiedName.contains(".")) { //$NON-NLS-1$
            ...
            if ("byte".equals(baseQualifiedName)) { //$NON-NLS-1$
                ...
                primitive = true;
                primitiveTypeWrapper = PrimitiveTypeWrapper.getLongTypeInstance();
