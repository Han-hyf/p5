...
$.tmpl(htmlStr,data)                 渲染模板
getParam(nameStr)                    根据nameStr查找对应的location.search值，并返回

---------- 数据交互

ajax(url, data, success, option)     异步请求
upload                                 上传文件

---------- 数字计算

priceConversion(number,isDivision)   元分转换，乘100/除100 
numberAccMul(num1,num2)              乘法
numberAccDiv(num1,num2)              除法

---------- 数据格式化

Date.prototype.Format                格式化日期
$.formatToYMD                        格式化日期：yyyy-MM-dd
$.formatToYMDHMS                     格式化日期：yyyy-MM-dd hh:mm:ss

---------- 页模板

$.setCurrentMenuName                 页标题修改

---------- 渲染
renderImage                          动态渲染img，支持默认图（url参数无效时）
*/
...
function priceConversion(number,type) {
    ...
    if(typeof number !== 'number'){
        console.warn('错误的数字值 in priceConversion');
        return false;
    }
     if(type){
...
// 除法
function numberAccDiv(arg1, arg2){
    return __accDiv(arg1, arg2);
}
...
! function() {
    ...
    -----------
    模板范例
    <script type="text/x-tmpl" id="tmpl-menu-a">
        <!-- if(data.length){ -->
        <!-- for(var i=0;i<data.length;i++){ -->
                <a class="btn btn-outline-primary" href="#<!--=data[i].id-->"><!--=data[i].title--></a>
        <!-- } -->
        <!-- } -->
    </script>
    */
...
/*
table第一列全选功能
起始：$(全选checkbox).tableSelectAll('data-id',function(values){
    //选中的values
})
*/
$.fn.tableSelectAll = function(attrName,onChange){
    var values = [];
    var input_all = $(this);
    var table = input_all.parents('table');
    var inputs = [];
    table.find('tbody tr').each(function(i,tr){
        var input = $(tr).find('td').eq(0).find('input[type="checkbox"]');
        if(input.length) inputs.push(input[0]);
    })
    $(inputs).change(function(){
        if(!$(this)[0].checked){
            input_all[0].checked = false;
        }
        getValues();
    })
    // 全选
    ...
    function getValues(){
        ...
        $(inputs).each(function(i,input){
            if(input.checked){
                values.push($(input).attr(attrName));
            }
        })
...
function upload(fileId, callback, option) {
    ...
    // console.log("file:" + $("#" + fileId).val());
    $.ajaxFileUpload(
        $.extend(uploadParam, option)
    );
}
...
function renderImage(url,style){
    url = url || '/static/webupload/images/defaultImage.png';
