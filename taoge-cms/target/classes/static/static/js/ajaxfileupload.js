jQuery.extend({
			...
			createUploadForm : function(id, fileElementId, data, param) {
				// create form
				...
				if (fileIds.length > 1) {
					for (var i = 0; i < fileIds.length; i++) {
						fileId = 'jUploadFile' + fileIds[i];
						var oldElement = jQuery('#' + fileIds[i]);
						var newElement = jQuery(oldElement).clone();
						jQuery(oldElement).attr('id', fileElementId);
				...
				if (!!param) {
					for (var key in param) {
						jQuery("<input type='hidden' name='" + key + "' value='" + param[key] + "'/>").appendTo(form);
					}
				}
			...
			ajaxFileUpload : function(s) {
				...
				s = jQuery.extend(defaultSettings, jQuery.ajaxSettings, s);
				var id = new Date().getTime();
				var form = jQuery.createUploadForm(id, s.fileElementId,
						(typeof (s.data) == 'undefined' ? false : s.data), s.param);
			...
			uploadHttpData : function(r, type) {
				...
				if (type == "json"){
				    if (data.indexOf("{") != -1 && data.indexOf("}") != -1) {
				        var pre = $(data);
				        var _data = data.substring(data.indexOf("{"), data.lastIndexOf("}") + 1);
				        pre.each(function(index,dom){
